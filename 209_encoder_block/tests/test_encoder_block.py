...
def test_matches_torch_fixture(path):
    ...
    out = encoder_block(d["x"], _params(d), int(d["n_heads"]), activation=act)
...
def test_shape_preserved():
    ...
    # also check silu path preserves shape
    out2 = encoder_block(d["x"], _params(d), int(d["n_heads"]), activation="silu")

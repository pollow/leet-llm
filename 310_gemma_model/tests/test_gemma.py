"""310 — tests for ``softcap`` / ``geglu_ffn`` and the Gemma-2 whole-model.
...
Categories:
  ...
  4. Real-weights parity (B, skippable) — ``gemma_forward`` vs ``real_ref.npz`` logits
     from a genuine ``Gemma2ForCausalLM``. Run ``310_gemma_model/download.sh`` first.
"""
...
@pytest.mark.skipif(
    ...
    reason="run 310_gemma_model/download.sh to fetch real weights",
)

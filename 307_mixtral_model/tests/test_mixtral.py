"""307 — tests for ``moe_ffn`` and the Mixtral whole-model.
...
Three categories:
  ...
  3. Real-weights parity (B, skippable) — ``mixtral_forward`` vs ``real_ref.npz`` logits
     ...
     Run ``307_mixtral_model/download.sh`` to populate the weights.
"""
...
@pytest.mark.skipif(
    ...
    reason="run 307_mixtral_model/download.sh to fetch real weights",
)

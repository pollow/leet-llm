...
def main() -> None:
    ...
    cfg = MarianConfig(
        ...
        forced_eos_token_id=None,  # None ⇒ HF greedy == pure argmax
    )
    model = MarianMTModel(cfg).double().eval()
    src = np.array([[5, 6, 7, 8, 0]])
    with torch.no_grad():
        gen = model.generate(
            torch.tensor(src), max_length=12, num_beams=1, do_sample=False
        )
    arrays = {k: v.detach().numpy() for k, v in model.state_dict().items()}
    arrays.update(
        src_ids=src,
        expected_ids=gen.numpy(),
        d_model=np.array(16),
        n_heads=np.array(4),
        n_enc_layers=np.array(2),
        n_dec_layers=np.array(2),
        d_ff=np.array(32),
        vocab_size=np.array(64),
        max_pos=np.array(32),
        scale_embedding=np.array(True),
        pad_id=np.array(63),
        eos_id=np.array(0),

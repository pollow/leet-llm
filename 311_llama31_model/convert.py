"""311 — fetch + convert llamafactory/tiny-random-Llama-3 to our .npz
(AUTHORING/DEMO, gen group).

    uv run --group gen python 311_llama31_model/convert.py

Downloads ONLY config.json + model.safetensors — no un-permute needed because
Llama-3.1 uses rotate-half RoPE, same layout as HF.  Unlike 310 (which forced the
checkpoint's RoPE to default) this checkpoint ships an **active** ``rope_type=llama3``
long-context schedule — exactly the delta 311 implements — so nothing is forced.

"""311 — Long-context RoPE scaling: the Llama-3.1 whole-model forward.
...
Two new delta operators plus the full Llama-3.1 decoder assembly:
...
- ``llama31_forward``: compose ``embedding`` (201) → per layer [``rms_norm`` (212)
  ...
  on q & k with the scaled ``inv_freq`` → repeat-kv → ``sdpa`` (205) with a
  ``triangular_mask`` (009) → ``ungroup_last_axis`` (001) + o_proj → ``add_residual``
  (208) → ``rms_norm`` → ``swiglu_ffn`` (214) → residual] → final ``rms_norm`` →
  ``@ lm_head.T``.  Use rotate-half RoPE, NOT ``llama_decoder_block`` (216,
  interleaved-only).  Compute ``inv_freq`` once via ``rope_scaled_freqs``.
"""
...
def rope_scaled_freqs(
    ...
    """
    raise NotImplementedError("Implement rope_scaled_freqs — see 311_llama31_model/README.md")
...
def rope_from_freqs(
    ...
    """
    raise NotImplementedError("Implement rope_from_freqs — see 311_llama31_model/README.md")
...
def load_llama31(weights: dict, cfg: Llama31Config) -> Llama31Params:
    ...
    """
    raise NotImplementedError("Implement load_llama31 — see 311_llama31_model/README.md")
...
def llama31_forward(
    ...
    Compose from granular L2 primitives (NOT ``llama_decoder_block``):
      ...
      head-split → ``rope_from_freqs`` on q & k → repeat-kv → ``sdpa`` with a
      causal ``triangular_mask`` → merge + o_proj → ``add_residual`` →
      ``rms_norm`` → ``swiglu_ffn`` → residual] → final ``rms_norm`` →
      ``@ lm_head.T``.

    RoPE is rotate-half (``rope_from_freqs``); long-context decode (the KV-cache
    side of the scaled schedule) is deferred to L4.
    """
    raise NotImplementedError("Implement llama31_forward — see 311_llama31_model/README.md")

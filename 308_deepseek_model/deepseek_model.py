"""308 — Multi-head Latent Attention (MLA) + DeepSeek-V3 whole-model forward.
...
- ``deepseek_forward``: compose ``embedding`` (201) → per layer [``rms_norm`` → MLA →
  ...
  top-k selection + routed_scaling_factor (reuse ``moe_ffn`` (307) for the expert
...
def mla_project(
    ...
    np.ndarray
        Output shape ``(B, L, d)``.
    """
    raise NotImplementedError("Implement mla_project — see 308_deepseek_model/README.md")
...
def load_deepseek(weights: dict, cfg: DeepseekConfig) -> DeepseekParams:
    ...
    """
    raise NotImplementedError("Implement load_deepseek — see 308_deepseek_model/README.md")
...
def deepseek_forward(
    ...
    ``moe_ffn`` (307) for expert dispatch + always-on shared experts (``swiglu_ffn``).
    """
    raise NotImplementedError("Implement deepseek_forward — see 308_deepseek_model/README.md")

"""308 — tests for ``mla_project`` and the DeepSeek-V3 whole-model forward.
...
Four categories:
  ...
  4. Real-weights parity (B, skippable) — ``deepseek_forward`` vs ``real_ref.npz``
     ...
     Run ``308_deepseek_model/download.sh`` to populate the weights.
"""
...
@pytest.mark.skipif(
    ...
    reason="run 308_deepseek_model/download.sh to fetch real weights",
)

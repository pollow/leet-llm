"""211 — generate frozen golden fixtures for the GPT block (pre-norm, decoder-only).

AUTHORING ONLY (needs the ``gen`` group):
    uv run --group gen python 211_gpt_block/tests/gen_fixtures.py

Oracle: float64 torch  h = x + SelfAttn(LN1(x), causal); y = h + FFN(LN2(h)).
"""
...
def main() -> None:
    ...
    B, L, d, d_ff, n_heads = 2, 5, 16, 32, 4
    arr = {
    ...
    h = t["x"] + _mha(_ln(t["x"], t["n1g"], t["n1b"]), attn_W, n_heads, mask=causal)
    y = h + _ffn(_ln(h, t["n2g"], t["n2b"]), ffn_W)
    arr["out"] = y.numpy()

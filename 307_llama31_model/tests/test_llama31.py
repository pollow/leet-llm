"""307 — tests for ``rope_scaled_freqs`` / ``rope_from_freqs`` and the Llama-3.1 whole-model.

Categories:
  1. Operator unit tests:
     - ``rope_scaled_freqs`` vs frozen genuine-HF ``inv_freq`` goldens (per ``rope_type``);
      ``default`` equals the plain 213 schedule.
  ...
  4. Real-weights parity (B, skippable) — ``llama31_forward`` vs ``real_ref.npz`` logits
     ...
     ships an active ``rope_type=llama3`` schedule). Run ``307_llama31_model/download.sh`` first.
"""
...
@pytest.mark.parametrize("rope_type", ["llama3"])
...
@pytest.mark.skipif(
    ...
    reason="run 307_llama31_model/download.sh to fetch real weights",
)

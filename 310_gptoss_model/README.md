...
2. **`gptoss_moe_ffn(...)`** — GPT-OSS's sparse MoE. It is **not** Mixtral's
   `moe_ffn` (307); see the contrast table below.
...
- **RoPE** — default rotate-half (`rope_half`). The real checkpoint declares a YaRN
  long-context schedule; that scaling is deferred (see → 311 / L4 below).

### GPT-OSS MoE vs Mixtral `moe_ffn` (307)

| | Mixtral `moe_ffn` (307) | GPT-OSS `gptoss_moe_ffn` (310) |
|---|---|---|
...
long-context scaling, deferred to 311 / L4).

> **→ 311 / L4:** GPT-OSS's real RoPE is YaRN long-context scaling. The `inv_freq`
> rescaling lives in 311 (Llama-3.1 long-context RoPE); long-context decode is an
...
```bash
uv run grade 310                  # grade your implementation
uv run grade 310 -v               # verbose output

# optional: real-weights parity (downloads a tiny checkpoint, ~MBs)
bash 310_gptoss_model/download.sh
```

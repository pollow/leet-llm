"""310 — Attention sinks + GPT-OSS MoE: GPT-OSS whole-model forward.
...
2. ``gptoss_moe_ffn(...)`` — GPT-OSS's sparse mixture-of-experts FFN.  It is NOT
   Mixtral's ``moe_ffn`` (307): the router has a bias, the gate softmax is taken
...
def attention_with_sinks(
    ...
    np.ndarray
        ...
        the sink column.
    """
    raise NotImplementedError("Implement attention_with_sinks — see 310_gptoss_model/README.md")
...
def gptoss_moe_ffn(
    ...
    Differs from Mixtral's ``moe_ffn`` (307) in every routing/expert detail:
    ...
    np.ndarray
        Same shape as ``x``.
    """
    raise NotImplementedError("Implement gptoss_moe_ffn — see 310_gptoss_model/README.md")
...
def load_gptoss(weights: dict, cfg: GptOssConfig) -> GptOssParams:
    ...
    """
    raise NotImplementedError("Implement load_gptoss — see 310_gptoss_model/README.md")
...
def gptoss_forward(
    ...
    GPT-OSS checkpoint's YaRN long-context scaling is deferred to 311 / L4.
    """
    raise NotImplementedError("Implement gptoss_forward — see 310_gptoss_model/README.md")

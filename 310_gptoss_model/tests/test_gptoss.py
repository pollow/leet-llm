"""310 — tests for ``attention_with_sinks`` / ``gptoss_moe_ffn`` and the GPT-OSS whole-model.
...
Categories:
  ...
  4. Real-weights parity (B, skippable) — ``gptoss_forward`` vs ``real_ref.npz`` logits
     from a genuine ``GptOssForCausalLM``. Run ``310_gptoss_model/download.sh`` first.
"""
...
@pytest.mark.skipif(
    ...
    reason="run 310_gptoss_model/download.sh to fetch real weights",
)
def test_gptoss_real_weights_logits():
    ...
    rope_type forced to 'default' — the checkpoint's YaRN scaling is deferred to 311).

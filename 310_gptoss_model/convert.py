"""310 — fetch + convert hf-internal-testing/tiny-random-GptOssForCausalLM
to our .npz (AUTHORING/DEMO, gen group).

    uv run --group gen python 310_gptoss_model/convert.py
...
Two forced settings (analogous to 307 forcing SiLU):
  ...
  * ``rope_type='default'`` — the checkpoint declares YaRN long-context scaling, which
    is deferred to 311 / L4.  Our forward uses default rotate-half RoPE, so we force the
